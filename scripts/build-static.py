...
body { font-family: var(--sans); font-weight: 400; min-height: 100%; overflow-x: hidden; }
...
header { position: sticky; top: 0; z-index: 50; background: var(--cream); padding-top: env(safe-area-inset-top); }
.header-row, .footer-row { display: flex; align-items: center; justify-content: space-between; gap: 1rem; }
.header-row { padding: 0.7rem 0; }
...
.mobile-nav a {
  ...
  padding: 0.85rem 1.25rem; font-size: 1.02rem; font-weight: 500;
...
.lang-switcher summary {
  ...
  min-width: 2.5rem; min-height: 2.75rem; gap: 0.2rem; cursor: pointer; opacity: 0.55;
}
...
.hero-copy { display: flex; flex-direction: column; justify-content: center; gap: 1.5rem; padding: 1.5rem 0 1.25rem; }
h1 { max-width: 14.5em; margin: 0; font-size: clamp(1.9rem, 8vw, 3.05rem); line-height: 1.16; text-wrap: pretty; }
...
@media (min-width: 768px) {
  ...
  .hero-copy { gap: 2rem; padding: 2rem 0 1.75rem; }
...
@media (max-width: 639px) {
  .wrap, .wrap-left { width: min(1220px, calc(100% - 2.5rem)); }
  .kicker { letter-spacing: 0.12em; }
  .pillars { gap: 1rem; padding-top: 1rem; }
  .principle { padding: 2.35rem 0; }
  .projects, .thesis, .about, .team, .portfolio { padding: 2.15rem 0 2.35rem; }
  .contact-copy { padding: 1.75rem 0 1.5rem; }
  .contact-open { display: flex; width: 100%; justify-content: center; box-sizing: border-box; }
  .contact-frame { height: 16.5rem; }
  .contact-modal { padding: 0.75rem; padding-bottom: max(0.75rem, env(safe-area-inset-bottom)); }
  .lang-link { min-height: 2.5rem; padding: 0.55rem 0.75rem; }
  .footer-row { flex-direction: column; align-items: flex-start; gap: 0.85rem; padding: 1.5rem 0 calc(1.5rem + env(safe-area-inset-bottom)); }
}
"""
